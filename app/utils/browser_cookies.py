...
def _find_cookie_db(user_data_dir: Path, profile_dir: str) -> Path:
    candidates = [
...
def export_storage_state_from_profile_id(profile_id: str) -> ExportResult:
    """Export a Playwright-style storage_state.json from an existing Edge/Chrome profile.

    This avoids launching Playwright and simply reuses cookies from the user's
    already logged-in browser profile.
    """
    ...
    cookie_db = _find_cookie_db(user_data_dir, profile_dir)
